#----------------------------------
# This code for those tweeters of rumor whose tweets only retweeted once! gives the following information:
# ratio of following/follower (mean, stdv)
# ratio of Mutal/followings (mean, stdv)
...
def main ():
    ...
    #-----------------
    #The specific area
    RT1_Analysis (G_Directed_with_Attributes,path_output)
#--------------------------------
...
#-------------------------------------------------------------------
def RT1_Analysis (G_Directed_with_Attributes,path_output) :
        ...
        for NI in G_Directed_with_Attributes.Nodes():
                ...
                if G_Directed_with_Attributes.GetStrAttrDatN(NI, "State_of_User"):
                        SUList = literal_eval(G_Directed_with_Attributes.GetStrAttrDatN(NI, "State_of_User"))
                        for SU in SUList :
                                if SU == 'RT' :
                                    SU_index = SUList.index (SU)
                                    NTOList = literal_eval(G_Directed_with_Attributes.GetStrAttrDatN(NI, "N_of_Tweet_Occurance"))
                                    if NTOList[SU_index] == '1':
                                        RT1_User_NAME = G_Directed_with_Attributes.GetStrAttrDatN(NI, "NAME_USERS")
                                        RT1_Date_Creation_Account = G_Directed_with_Attributes.GetStrAttrDatN(NI, "Date_Creation_Account")
                                        RT1_N_Posts = G_Directed_with_Attributes.GetStrAttrDatN(NI, "N_Tweets")
                                        RT1_N_Posts_per_Year = (int (RT1_N_Posts))/(2019 - int (RT1_Date_Creation_Account))
                                        RT1_N_Followers = G_Directed_with_Attributes.GetStrAttrDatN(NI, "N_Followers")
                                        RT1_N_Friends = G_Directed_with_Attributes.GetStrAttrDatN(NI, "N_Friends")
                                        if RT1_N_Followers == '0':
                                            RT1_N_Followers = '1'
                                        RT1_N_Friends_To_N_Followers = (float (RT1_N_Friends))/(float (RT1_N_Followers))
                                        RT1_ID_in_Graph = NI.GetId()
                                        MUTUAL = 0
                                        for NIN in G_Directed_with_Attributes.Nodes():  
                                                if (NIN.IsOutNId(RT1_ID_in_Graph)) and (NIN.IsInNId(RT1_ID_in_Graph)) :
                                                        MUTUAL += 1
                                        if float (NI.GetOutDeg()) != 0 :
                                                RT1_Mutal_To_N_Friends = (float (MUTUAL)) / (float (NI.GetOutDeg()))
                                        else:
                                                RT1_Mutal_To_N_Friends = None
                                        DateTimeList = literal_eval(G_Directed_with_Attributes.GetStrAttrDatN(NI, "Date_Time"))
                                        RT1_Date_Time = DateTimeList [SU_index]
                                                                                        
                                        UnitData = [] 
                                         
                                        UnitData.append (RT1_N_Friends_To_N_Followers)
                                        UnitData.append (RT1_Mutal_To_N_Friends)
                                        UnitData.append (RT1_N_Posts_per_Year)
                                        UnitData.append (RT1_Date_Time) 
        ...
        RT1 = tuple (RT1)
        text_file = open(path_output + "S18_2_Output.txt", "a")
        text_file.write('######################################' "\n")
        text_file.write('\n'"===========")
        text_file.write('\n'"RT1 Characteristics " '\n')            
        if [float (i[0]) for i in RT1] :
                text_file.write("mean_fw_to_fr : " "%s\n" %  statistics.mean([float (i[0]) for i in RT1]))
                if len ([float (i[0]) for i in RT1]) > 1 :
                        text_file.write("stdev_fw_to_fr : " "%s\n" %  statistics.stdev([float (i[0]) for i in RT1]))
        if [float (i[1]) for i in RT1 if i[1]] :
                text_file.write("mean_mutal_to_fw : " "%s\n" %  statistics.mean([float (i[1]) for i in RT1 if i[1]]))
                if len ([float (i[1]) for i in RT1 if i[1]]) > 1 :
                        text_file.write("stdv_mutal_to_fw : " "%s\n" %  statistics.stdev([float (i[1]) for i in RT1 if i[1]]))
        if [float (i[2]) for i in RT1] :
                text_file.write("mean N_Tweets/Year : " "%s\n" % statistics.mean([float (i[2]) for i in RT1]))
                if len ([float (i[2]) for i in RT1]) > 1 :
                        text_file.write("stdv N_Tweets/Year : " "%s\n" % statistics.stdev([float (i[2]) for i in RT1]))
        if [float (i[3]) for i in RT1] :
                text_file.write("mean Date_Time : " "%s\n" % statistics.mean([float (i[3]) for i in RT1]))
                if len ([float (i[3]) for i in RT1]) > 1 :
                        text_file.write("stdv Date_Time : " "%s\n" % statistics.stdev([float (i[3]) for i in RT1])) 
        text_file.write('\n'"===========")
        text_file.write('\n'"######################################")
